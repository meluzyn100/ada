...
<<results= 'asis', echo = F>>=
tab2 <- xtable(data2,caption = 'Tabela do zadania 3.' ,label = "tab:1")
...
\subsubsection*{Współczynnik \(\tau\) Goodmana}
\begin{gather*}
...
Bedziemy również wykożystywać trzy inne współczynniki oparte na statystyce \(\chi^2\) są to: współczynnik V-Cramera, \( \varphi \), oraz C-Pearsona opisane poniższymi wzorami.
\begin{gather*}
  V = \sqrt{ \frac{\chi^2}{n \min(R-1,C-1)} },\quad
  \varphi = \sqrt{\frac{\chi^2}{n}},\quad
...
c("    A",    "B",    "C",          "D"))
@
Tabela z przykładu 1 wykład 7, opis kategori:
\begin{enumerate}
 \item[A.] Segreguję śmieci, ponieważ jest to korzystne dla środowiska,
 \item[B.] Segreguję śmieci, ponieważ taki jet wymóg ustawowy,
 \item[C.] Segreguję śmieci, ponieważ wsszyscy tak robią,
 \item[D.] Nie segreguję śmieci.
\end{enumerate}


<<results= 'asis', echo = F>>=
tab4 <- xtable(data4, caption = 'Dane do listy 8 i 9, zadanie 1.' ,label = "tab:4")
align(tab4) <- xalign(tab4)
digits(tab4) <- xdigits(tab4)
display(tab4) <- xdisplay(tab4)
addtorow <- list()
addtorow$pos <- list(0, 0)
addtorow$command <- c("& \\multicolumn{4}{c}{Kategoria} \\\\\n",
	"Wiek & A & B & C & D \\\\\n")
print(tab4, scalebox = "1", add.to.row = addtorow, include.colnames = FALSE,table.placement="H")
@
...
<<fig_1, fig.width = 10, fig.height = 6, fig.cap ='\\label{fig:1}Analiza korespondecji dla tabeli za Przykładu 1 Wykład 7.' ,fig.pos = 'H'>>=
...
<<results= 'asis', echo = F>>=
tab5 <- xtable(data5, caption = 'Dane do listy 8 i 9, zadanie 2.' ,label = "tab:5")
...
cat('tau =',tau(data5)[[1]])
@
Jak możemy zobaczyć otrzymana wartość współczynnika \(\tau\) nie jest zbyt duża, świadczy to o średniej współzmienności badanych zmiennych losowych.

<<warning = F>>=
cat(wspolczynniki(data5,'Cramer'))
cat(wspolczynniki(data5,'t-czuprow'))
cat(wspolczynniki(data5,'phi'))
cat(wspolczynniki(data5,'pearson'))
@

Tabela w tym zadaniu jest rozmiaru 2x2, więc powinniśmy rozpatrzyć tylko współczynnik $\tau$.Jak możemy zobaczyć otrzymana wartość tego współczynnika wskazuję na to, że zmienna \emph{Lek} ma wpływ na redukcję zmienności zmiennej \emph{Wiek}.
...
<<results= 'asis', echo = F>>=
tab6 <- xtable(data6, caption = 'Dane do listy 8 i 9, zadanie 3.' ,label = "tab:6")
...
Jak możemy zobaczyć otrzymana wartosc współcznnika \(\tau\) jest bliska zeru, świadczy to o bardzo małej współzmienności badanych zmiennych losowych.
