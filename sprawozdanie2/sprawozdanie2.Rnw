...
tab <- xtable(zad2_df, digits=c(0,5,5,5,5,5),,caption = 'Tablicza wartości współczynników dla poszczegulnych macieży dla adania 1.' ,label = "tab:4" )
...
<<warning = F, results= 'asis', echo = F>>=
zad2_df <- matrix(c(
  TschuprowT(data5), wspolczynniki(data5,'t-czuprow'),
     CramerV(data5), wspolczynniki(data5,'Cramer'),
         Phi(data5), wspolczynniki(data5,'phi'),
    ContCoef(data5), wspolczynniki(data5,'pearson')),c(2,4))
# rownames(zad2_df) <- c("f. wbudowana", "wartość miary")

# colnames(zad2_df) <- c("$\\tau$","V","$\\varphi $","C") 
tab <- xtable(zad2_df)
print(tab, sanitize.text.function = function(x) {x})
@
