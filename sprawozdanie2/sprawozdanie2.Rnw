   ...
   (X/(X+n))**0.5
    ...
    for (i in 1:nrow(dane)-1){
      for (j in 1:ncol(dane)-1){
        a <- a + sum(dane[i,j]) * sum(dane[min(i+1,nrow(dane)):nrow(dane),
    ...
    for (i in 1:nrow(dane)-1){
      for (j in ncol(dane):2){
        b <- b + sum(dane[i,j]) * sum(dane[min(i+1,nrow(dane)):nrow(dane),
                                           max(j-1,1):1])
      }
    }
    (a-b)/(a+b)
...
<<>>=
inercja_calkowita <- function(data) {
  p <- data/sum(data)
  r <- rowSums(p)
  c <- colSums(p)
  R <- nrow(data)
...
% <<>>=
% (inercja_calkowita(m1b)[1]+inercja_calkowita(m1b)[2]
% )/sum(inercja_calkowita(m1b))
% inercja_calkowita(m1b)
% inercja_calkowita(m1b)[1]
% inercja_calkowita(m1b)[2]
% inercja_calkowita(m1b)[1]+inercja_calkowita(m1b)[2]
% sum(inercja_calkowita(m1b))
...
<<warning = F, results= 'asis', echo = F>>=
zad2_df <- matrix(c(inercja_calkowita(m1a)[1],inercja_calkowita(m1a)[2],
inercja_calkowita(m1b)[1],inercja_calkowita(m1b)[2],
inercja_calkowita(m1c)[1],inercja_calkowita(m1c)[2],
inercja_calkowita(m2a)[1],inercja_calkowita(m2a)[2]),
           c(4,2),byrow = T)
rownames(zad2_df) <- c("m1a", "m1b", "m1c", "m2a")
colnames(zad2_df) <- c("$\\lambda$","inercja cz.") 
tab <- xtable(zad2_df, digits=c(0,5,5),caption = 'Tablicza wartości współczynników dla poszczegulnych macieży dla adania 1.' ,label = "tab:5" )
print(tab, sanitize.text.function = function(x) {x},table.placement="H")
@
...
tab_l2zad2_df <- xtable(l2zad2_df, digits=c(0,5,5,5,5,5),caption = 'Tablica wartości współczynników dla zadania 2.' ,label = "tab:6" )
...
<<results= 'asis', echo = F>>=
tab6 <- xtable(data6, caption = 'Dane do listy 8 i 9, zadanie 3.' ,label = "tab:7")
...
tab_l2zad3_df <- xtable(l2zad3_df, digits=c(0,5,5,5,5,5,5),caption = 'Tablica wartości współczynników dla zadania 3.' ,label = "tab:8" )
...
Ze względu na to ze badana tabela zawiera dwie zmienne kategoryczne i uporządkowane, możemy w tym przypadku posłużyć się dodatkowym spółczynnikiem \( \hat\gamma \)

<<fig_3, fig.width = 10, fig.height = 6, fig.cap ='\\label{fig:3}Analiza korespondecji dla Tabeli 3.' ,fig.pos = 'H'>>=
df3 <-analiza(data6)
