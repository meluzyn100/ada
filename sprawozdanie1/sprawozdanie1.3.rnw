...
ggplot(df_l,aes(p, value,col=variable))+
  geom_point()+ geom_line()
@
Zobaczmy również jak wygladaja powyższe wykresy, ale tym razem dla n równego 100:
<<echo = FALSE>>=
...
\newline
Do wyboru najlepszych przedziałów ufności stosujemy funkcję $binom.confint$. Funkcja ta zwraca tabelkę z porównaniem 11 metod. Przedziały będziemy porównywać na podstawie ich długości, im węższy tym metoda jest lepsza. Długość takiego przedziału bedzie opisana w tabelce w kolumnie $ls$.\newline
